...
function generateScenarios(emp: Employee, decision: string, timeframe: string): Scenario[] {
  ...
  if (decision === "Relocate to Munich") {
    return [
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          ...
          <button
            onClick={simulate}
            disabled={!selectedEmp || !decision || !timeframe}
            className="w-full py-3 rounded-lg text-sm font-bold bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed transition-all animate-pulse-glow"
          >
            Simulate →
          </button>
        </div>
        ...
        <div className="space-y-5">
            ...
            {scenarios
              ? scenarios.map((s, i) => (
                  <div key={i} className="bg-card border border-border rounded-xl p-4 hover:border-primary/30 transition-all">
                    ...
                    <p className="text-sm font-medium text-gold mb-2">{s.cost}</p>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              {advice || "Configure and run a simulation above to receive AI-powered recommendations."}
            </p>
          </div>
        </div>
      </div>
